...
class ModelToolImage extends Model {
    public function resize($filename, $width, $height) {
        if (!is_file(DIR_IMAGE . $filename) || substr(str_replace('\\', '/', realpath(DIR_IMAGE . $filename)), 0, strlen(realpath(DIR_IMAGE))) != str_replace('\\', '/', realpath(DIR_IMAGE))) {
            return;
        }
        ...
        return $this->config->get('config_url') . 'media/image/' . $image_new;
    }
}
