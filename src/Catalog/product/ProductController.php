    ...
    public function legacyIndex(Request $request): Response {
        ...
        if ($request->query->has('path') === true) {
            ...
            if ($category_info) {
                ...
                if ($request->query->has('sort') === true) {
                    $url .= '&sort=' . $request->query->get('sort');
                }

                if ($request->query->has('order') === true) {
                    $url .= '&order=' . $request->query->get('order');
                }

                if ($request->query->has('page') === true) {
                    $url .= '&page=' . $request->query->get('page');
                }

                if (isset($this->request->get['limit'])) {
                    $url .= '&limit=' . $request->query->get('limit');
                }

                $data['breadcrumbs'][] = array(
                    'text' => $category_info['name'],
                    'href' => $this->url->link('product/category', 'language=' . $this->config->get('config_language') . '&path=' . $request->query->get('path') . $url)
                );
            }
        }

        if ($request->query->has('manufacturer_id') === true) {
            $data['breadcrumbs'][] = array(
            ...
            $manufacturer_info = $this->manufacturerRepo->getManufacturer($this->request->get['manufacturer_id']);
        ...
        if ($product_info) {
            ...
            $results = $this->productRepo->getProductRelated($this->request->get['product_id']);
